...
- **Staging**: a branch git `staging` (a partir de `main`) tem deploy automático de Preview na Vercel em **https://iluminus-saas-git-staging-iluminussaas-3261s-projects.vercel.app** (URL fixa, atualiza a cada push na branch); as env vars `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` estão sobrescritas nela (escopo Preview + git branch `staging`) para apontar ao projeto Supabase de staging acima, em vez de produção — confirmado inspecionando o bundle buildado. As demais env vars (ex.: VAPID key) continuam vindo do escopo Preview geral. Não existe projeto Vercel separado para staging — é a mesma `iluminus-saas`, só uma branch com env vars diferentes. Nota: um push que só avança o ponteiro da branch para um commit já existente/já buildado em outro lugar (ex.: fast-forward de `main`) não gera um novo deploy — precisa ser um commit genuinamente novo naquela branch (mesmo um `git commit --allow-empty` serve para forçar um redeploy).
