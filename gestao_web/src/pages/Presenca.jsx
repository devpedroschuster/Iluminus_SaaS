...
export default function Presenca() {
  ...
  function calcularMetricas(presencasData, alunosData) {
    ...
    // Calcular frequência média semanal
  ...
  async function realizarCheckin(aluno, aulaId = null) {
    try {
      // Verificar se já fez check-in hoje na mesma aula
      const hoje = new Date().toISOString().split('T')[0];
  ...
  async function visualizarDetalhes(aluno) {
    try {
      // Buscar histórico do aluno nos últimos 30 dias
  ...
  function exportarRelatorio() {
    const dadosExport = presencasFiltradas.map(p => ({
      ...
      'Aula': p.agendas?.atividade || 'Treino Livre',
      'Horário': p.agendas?.horario || '-',
      'Tipo': p.tipo
    }));
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
              ...
              return (
                <div key={idx} className="flex-1 flex flex-col items-center gap-2">
                  <div className="flex-1 flex items-end w-full">
                    <div 
                      className="bg-iluminus-terracota rounded-t-xl w-full transition-all hover:brightness-95"
      ...
      <div className="bg-white p-6 rounded-[28px] border border-gray-100 shadow-sm">
        <div className="flex flex-wrap gap-4">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${
                  filtros.periodo === periodo 
                    ? 'bg-iluminus-terracota text-white' 
          ...
          >
            ...
            {aulas.map(a => (
              <option key={a.id} value={a.id}>{a.atividade} - {a.horario}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm overflow-hidden">
        {loading ? <TableSkeleton /> : presencasFiltradas.length === 0 ? (
          <EmptyState 
            ...
            mensagem="Faça o primeiro check-in do dia!"
          />
        ) : (
                ...
                <tr key={p.id} className="hover:bg-gray-50/30 transition-colors">
                  ...
                  <td className="px-8 py-5">
                    <span className="text-sm font-medium text-gray-600">
                      {p.agendas?.atividade || 'Treino Livre'}
                    </span>
                    {p.agendas?.horario && (
                      <p className="text-xs text-gray-400">{p.agendas.horario}</p>
                    )}
                  </td>
                  ...
                  <td className="px-8 py-5 text-right">
                    <button
                      ...
                      className="bg-white border border-gray-100 px-4 py-2 rounded-xl font-bold text-xs text-gray-600 hover:bg-gray-50 transition-all"
                    >
                      Ver Histórico
                    </button>
                  </td>
      ...
      <Modal
        isOpen={modalCheckin.isOpen}
        onClose={modalCheckin.fechar}
            ...
            {alunosFiltrados.length === 0 ? (
              ...
              alunosFiltrados.map(aluno => (
                <button
                  ...
                  className="w-full p-4 bg-white border border-gray-100 rounded-2xl hover:border-iluminus-terracota hover:bg-orange-50/30 transition-all text-left"
                >
      ...
      {alunoSelecionado && (
        <Modal
          ...
          onClose={modalDetalhes.fechar}
          titulo={`Histórico: ${alunoSelecionado.nome_completo}`}
        >
          <div className="space-y-6 pt-2">
            {/* Estatísticas */}
                    ...
                    <div key={h.id} className="p-3 bg-gray-50 rounded-xl flex justify-between items-center">
                      <div>
                        <p className="font-bold text-sm text-gray-700">
                          {h.agendas?.atividade || 'Treino Livre'}
                        </p>
                        <p className="text-xs text-gray-400">
                          {new Date(h.data_checkin).toLocaleDateString('pt-BR')} às {new Date(h.data_checkin).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </div>
      ...
      <Modal
        ...
        onClose={modalQRCode.fechar}
        titulo="QR Code para Check-in"
      >
